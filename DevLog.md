...
- **Latest (2026-07-31): v3.0.1 — program history for every school, scoreboard filters, shortcuts, live OVR.** `package.json` 3.0.0 → 3.0.1 (+ lock), manual regenerated, `.backup/v3.0.1-<stamp>/`, tag `v3.0.1`, app-only slim rebuild (no artwork changed, so the 3.0.0 image pack stays valid). **Program history (the headline).** The save carries real pre-loaded history for all 138 programs and the app showed none of it — a browsed team's History page said "no titles in the tracked years". New `extract-team-history.ts` reads three things per Team: `TeamHistoricalData` (29 all-time totals), `TeamSeriesHistory` → up to 108 `TeamHistoricSeriesYear` rows, and `Career`/`SeasonStatRecords` → 9 named records each. Auburn now reads 791-480-47 with Bo Jackson's 4,303 (1985) — and its single-season passing record already belongs to a 2026 dynasty QB, which is the point. **Attribution is by REFERENCE, never row order**: `TeamHistoricSeriesYear` is one shared 4,140-row pool whose rows are NOT in teamIndex order (row 0 = Akron, row 1 = Air Force, while teamIndex 0 = Air Force), so positional reads produce a plausible page belonging to the wrong school. Two bugs found by running the pipeline: the array CONTAINER table (`TeamHistoricSeriesYear[]`) needs preloading as well as the element pool (without it seasons came back silently `[]` while all-time looked perfect), and `viewedTeamIndex` is null on your OWN program so the user's team would have been the only one with no history. **Live OVR** — EA's PORC weights baked into `ovrFormulas.generated.ts` from a gitignored reference; keyed on numeric (PPOS, PLTY) because the 2026 workbook's archetype NAMES don't survive into 27 and 52 of 93 rows are "Unlabeled in source". 99.97% exact on 14,838 supported players; K/P/FS declined outright (their weights changed — a linear refit only reaches 8-20%, so it's the weights not the bounds). **Recruits stopped masquerading as players**: global search returned 4,100 prospects labelled "FCS West" with their OVR printed, straight through the hidden-ratings rule; `teamIndex 255` is NOT the test (shared bucket, ~4,525 entities vs ~4,100 recruits) — membership of the recruit pool is. Guard sits in `PlayerModalProvider`, the one door every caller uses. **Shortcuts** — 24 destinations, none bound (deliberate), recorded not typed, reserved combos refused by name, never fires while typing. **Scores** — All NCAA / Top 25 / Upsets / per-conference filters with live counts, plus the game's own mark (bowl / playoff / conf championship / rivalry / conference) large and centred; conference title games derived via `shared/championshipWeek.ts`, not a week literal. **Nav fixes**: section membership now derived from each hub's tab list (four NCAA pages had been lighting "Coach"), and one shared `usePinnedSubNav` hook makes every hub's sub-nav behave like Team Hub's. **Standings**: headline/blurb removed and the conference mark was UNBOUNDED (`h-full` against a min-height parent → the SVG's natural size drove a 340px row); a definite tile height fixes the cause, mark now 82px.

...
- **v2.89** (2026-07-19): **Recruiting map crash fix + All Recruits filter board + Team Needs.** Map: zoom buttons moved below the box (were overlapping the northeast); drag switched from mouse events to Pointer Events + setPointerCapture (the real fix for the "drag to edge → blank screen" report — capture guarantees the drag session stays well-formed regardless of where the cursor goes); generous pan slack + overflow-hidden mask + NaN-safe clamping. Recruiting page: the five per-stage tables replaced with one filterable/sortable board (stage, stars, position, state, style, class + search) defaulting to showing every recruit. New Team Needs panel from real save data (Team.TEAM_RATING* position-group grades, the same numbers behind the in-game Team Ratings screen) added to the already-leaguewide extract-teams.ts read — zero new snapshot/IPC. Verified live against real board data. Details in DevLog.
...
- **v2.85** (2026-07-19): **Dynasty Trends dashboard + Transfer Portal tracker** (two Team Hub sub-tabs, built from the Ideas board). Trends: four theme-aware inline-SVG charts (W-L by season, weekly poll trajectory, recruiting-rank trend, points for/against) via the dataviz skill with a validated categorical palette, hover tooltips, inverted-y ranks. Transfers: diffs consecutive per-season league rosters by stable PresentationId to detect school changes (in/out, any team via the switcher, bio links). Investigation-first (id stability confirmed), verified against a disposable multi-season archive in both themes. Details in DevLog.
